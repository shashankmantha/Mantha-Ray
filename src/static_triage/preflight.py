"""Non-mutating discovery of external analysis tools."""

from __future__ import annotations

import shutil

from .models import ToolInfo
from .runner import run_command


TOOL_VERSION_ARGUMENTS: dict[str, tuple[str, ...]] = {
    "clamscan": ("--version",),
    "capa": ("--version",),
    "floss": ("--version",),
}


def inspect_tools(
    timeout_seconds: float = 5.0,
    max_output_chars: int = 2_000,
) -> list[ToolInfo]:
    """Check analyzer availability without scanning samples."""

    if timeout_seconds <= 0:
        raise ValueError("timeout_seconds must be positive")

    if max_output_chars <= 0:
        raise ValueError("max_output_chars must be positive")

    tools: list[ToolInfo] = []

    for name, version_arguments in (
        TOOL_VERSION_ARGUMENTS.items()
    ):
        executable = shutil.which(name)

        if executable is None:
            tools.append(
                ToolInfo(
                    name=name,
                    executable=None,
                    available=False,
                    error="not found on PATH",
                )
            )
            continue

        try:
            result = run_command(
                [executable, *version_arguments],
                timeout_seconds=timeout_seconds,
                max_output_bytes=max_output_chars * 4,
                                env={
                    "PATH": (
                        "/opt/floss-venv/bin:"
                        "/opt/capa-venv/bin:"
                        "/usr/local/bin:/usr/bin:/bin"
                    ),
                    "LC_ALL": "C",
                    "LANG": "C",
                    "HOME": "/tmp/preflight-home",
                    "XDG_CACHE_HOME": (
                        "/tmp/preflight-cache"
                    ),
                    "XDG_CONFIG_HOME": (
                        "/tmp/preflight-config"
                    ),
                    "USER": "static-triage",
                    "LOGNAME": "static-triage",
                },
            )

            combined_output = "\n".join(
                part
                for part in (
                    result.stdout,
                    result.stderr,
                )
                if part
            )

            version = (
                combined_output[:max_output_chars].strip()
                or None
            )

            if result.timed_out:
                error = "version check timed out"
            elif result.return_code != 0:
                error = (
                    "version command exited "
                    f"{result.return_code}"
                )
            else:
                error = None

            tools.append(
                ToolInfo(
                    name=name,
                    executable=executable,
                    available=error is None,
                    version=version,
                    error=error,
                )
            )

        except OSError as exc:
            tools.append(
                ToolInfo(
                    name=name,
                    executable=executable,
                    available=False,
                    error=(
                        "version check failed: "
                        f"{type(exc).__name__}"
                    ),
                )
            )

    return tools